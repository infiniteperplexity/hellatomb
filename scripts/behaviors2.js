...
HTomb = (function(HTomb) {
  ...
  HTomb.Things.defineBehavior({
    ...
    construct: function(x,y,z) {
      ...
      if (f===undefined) {
        for (var ingredient in this.task.ingredients) {
          var n = this.task.ingredients[ingredient];
          //remove n ingredients from the entity's inventory
          var taken = this.entity.inventory.items.take(ingredient,n);
          taken.remove();
        }
        f = HTomb.Things.Construction(this.task.featureTemplate);
        f.taskTemplate = this.task.template;
        f.place(x,y,z);
    ...
    requires: function(args) {
      ...
      if (f && f.taskTemplate===task.template) {
        return false;
      }
      args = args || {};
      var n;
      var t = cr.ai.target;
      // okay this actually does it wrong...
