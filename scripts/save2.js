...
HTomb = (function(HTomb) {
  ...
  HTomb.Save.saveGame = function() {
    ...
    nThings = HTomb.World.things.length;
    let json = HTomb.Save.stringify(HTomb.World.things);
  ...
  HTomb.Save.stringify = function(obj) {
    document.getElementById("ppp").innerHTML = "testing";
    let json = JSON.stringify(obj, function(key, val) {
      ...
      if (val.stringify) {
        ...
        if (nThings%100===0) {
            HTomb.GUI.pushMessage("Things left: " + nThings);
        }
        nThings-=1;
        seen.push(val);
      //} else if (arg===true) {
