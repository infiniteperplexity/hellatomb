...
public class RagerMasterAI extends MasterCritterAI implements CritterListener{
    CritterAI myCurrentAI;
    Critter myEnemy;
    DungeonSquare d;
    
    public RagerMasterAI(Critter c) {
        super(c);
    }
    ...
    public CritterAI nextAI(CritterAI current) {
        ...
        else if (current instanceof RagerAssaultAI && !current.isFinished())
            return current;
        else if (myEnemy!=null)
            return new RagerAssaultAI(myCritter, myEnemy);
